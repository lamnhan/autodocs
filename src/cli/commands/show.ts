...
export class ShowCommand {
  ...
  run(input?: string) { 
    ...
    if (!!SHORT_TEXT) {
      table.push([
        'Description', magenta('SHORT_TEXT'), green(SHORT_TEXT)
      ]);
    }
    if (!!TEXT) {
      table.push([
        'Content', magenta('TEXT'), green(content)
      ]);
    }
    if (!!Object.keys(SECTIONS).length) {
      table.push([
        'Sections', magenta('SECTIONS'), green(sections.join('\r\n'))
      ]);
    }
    if (!!RETURNS) {
      table.push([
        'Returns', magenta('RETURNS'), green(RETURNS)
      ]);
    }
    if (!!PARAMETERS.length) {
      table.push([
        'Params', magenta('PARAMETERS'), green(params.join(', '))
      ]);
    }
    if (!!DEFAULT_VALUE) {
      table.push([
        'Value', magenta('DEFAULT_VALUE'), green(DEFAULT_VALUE)
      ]);
    }
    // render
