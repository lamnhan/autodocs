...
export class Converter {
  ...
  convert(declaration: Declaration, output: string, options?: ConvertOptions) {
    switch (output) {
      ...
      case 'summary_interfaces':
        return this.convertInterfacesSummary(
          declaration.getInterfaces()
        );
      case 'summary_classes':
        return this.convertClassesSummary(
          declaration.getClasses()
        );
      case 'self':
      default:
  ...
  private convertSelf(declaration: Declaration) {
    ...
    const { LEVEL, ID, NAME, LINK, TYPE, TYPE_LINK, SHORT_TEXT, TEXT, RETURNS, PARAMETERS } = declaration;
    // default blocks
    ...
    if (declaration.isKind('CallSignature')) {
      ...
      // returns
      const displayType = !!TYPE_LINK ? `[\`${TYPE}\`](${TYPE_LINK})` : `\`${TYPE}\``;
      blocks.push(
  ...
  private convertFull(declaration: Declaration) {
    ...
    const variablesOrPropertiesFull = (
      declaration.hasVariablesOrProperties()
      ? this.convertVariablesOrPropertiesFull(declaration)
      : []
    );
    // functions or methods
    const functionsOrMethodsFull = (
      declaration.hasFunctionsOrMethods()
  ...
  private convertVariablesOrPropertiesSummary(declarations: Declaration[], standalone = true) {
    const blocks: Block[] = [];
    // get data
    const summaryRows: string[][] = [];
    declarations.forEach(declaration => {
      // summary
      const { NAME, LINK, IS_OPTIONAL, TYPE, TYPE_LINK, SHORT_TEXT } = declaration;
  ...
  private convertVariablesOrPropertiesFull(declaration: Declaration) {
    ...
    const summaryBlocks = this.convertVariablesOrPropertiesSummary(children, false);
    // detail
    const detailText = this.$Content.buildText(`**${parentName} ${t2} detail**`);
  ...
  private convertFunctionsOrMethodsSummary(executables: Declaration[], standalone = true) {
    const blocks: Block[] = [];
    // get data
    const summaryRows: string[][] = [];
  ...
  private convertFunctionsOrMethodsFull(declaration: Declaration) {
    ...
    // summary
    const summaryText = this.$Content.buildText(`**${parentName} ${t1}**`);
    const summaryBlocks = this.convertFunctionsOrMethodsSummary(children, false);
    // detail
    const detailText = this.$Content.buildText(`**${parentName} ${t2} detail**`);
  ...
  private convertInterfacesSummary(interfaces: Declaration[], standalone = true) {
    const blocks: Block[] = [];
    // get data
    const summaryRows: string[][] = [];
